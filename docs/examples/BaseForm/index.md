...
<template>
  <el-space
    ...
    fill
  >
    <BaseForm ref="formRef" :formList="formList" :cols="2" />
...
<template>
  <el-space direction="vertical" alignment="start" :size="20">
    <h3>一行展示{{ cols }}个</h3>
    <BaseForm ref="formRef" :formList="formList" :cols="cols" />
    <pre>{{ pageData }}</pre>
    ...
    <el-button
      ...
      @click="
        () => {
          if (formList[0].type === 'MOBILEINPUT') {
            formList[0].type = 'INPUT';
            formList[0].label = '输入框';
          } else {
            formList[0].type = 'MOBILEINPUT';
